...
contract Game {
    ...
    function addPlayer(string memory fullName, uint age, string memory sex) public {
        players[msg.sender] = Player(fullName, age, sex, Level.Beginner, block.timestamp);
        countPlayer += 1;
    }
    ...
    function changePlayerLevel(address player) public {
        Player storage player = players[player];
        if (block.timestamp >= player.createTime + 15) { //After 15s
            player.myLevel = Level.Intermadiate;
        }
    }
}